...
def _print_xml_element_header(element, attributes, stream, indentation=''):
  ...
  Args:
    ...
    attributes: ordered dictionary with attributes already escaped
  ...
  for name in attributes:
    if attributes[name] is not None:
      stream.write(' %s="%s"' % (name, attributes[name]))
  stream.write('>\n')
...
class _TestCaseResult(object):
  ...
  def print_xml_summary(self, stream):
    ...
    test_case_attributes = collections.OrderedDict([
        ('name', '%s' % self.name),
        ('status', '%s' % status),
        ('result', '%s' % result),
...
class _TestSuiteResult(object):
  ...
  def print_xml_summary(self, stream):
    ...
    overall_attributes = collections.OrderedDict([
        ('name', ''),
    ...
    for suite_name in self.suites:
      ...
      suite_attributes = collections.OrderedDict([
          ('name', '%s' % suite_name),
          ('tests', '%d' % len(suite)),
          ('failures', '%d' % failures),
          ('errors', '%d' % errors),
...
class _TextAndXMLTestResult(_pretty_print_reporter.TextTestResult):
  ...
  def startTestRun(self):
    self.suite.overall_start_time = self.time_getter()
    super(_TextAndXMLTestResult, self).startTestRun()

  def stopTestRun(self):
    self.suite.overall_end_time = self.time_getter()
