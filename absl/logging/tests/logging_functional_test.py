...
def _munge_log(buf):
  ...
  # Timestamp
  buf = re.sub(
      r'(?m)' + logging.ABSL_LOGGING_PREFIX_REGEX,
  ...
  buf = re.sub(
      r'(?sm)^\*\*\* Signal 6 received by PID.*\n.*', r'Stack trace:\n', buf
  )
  buf = re.sub(
      (
          r'(?sm)^\*\*\* ([A-Z]+) received by PID (\d+) '
          r'\(TID 0x([0-9a-f]+)\)'
  ...
  buf = re.sub(
      r'(?sm)^\*\*\* Check failure stack trace: \*\*\*\n.*',
      r'Stack trace:\n',
      buf,
  )
  ...
  if os.name == 'nt':
    ...
    buf = re.sub(
        r'(  File ").*(logging_functional_test_helper\.py", line )',
        r'\1\2',
        buf,
    )
